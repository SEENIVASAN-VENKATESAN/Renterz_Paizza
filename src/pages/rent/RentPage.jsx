...
export default function RentPage() {
  ...
  const adminRents = useMemo(() => {
    ...
    if (sortFilter === 'LATE_DESC') {
      records.sort((a, b) => calculateRemainingDays(a.dueDate) - calculateRemainingDays(b.dueDate))
    }
    return records
  }, [isOwner, isTenant, rents, sortFilter, statusFilter])
  ...
  const nextDueDate = useMemo(() => {
    ...
    return [...tenantRents]
      .sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime())
      .find((item) => new Date(item.dueDate).getTime() >= currentTime)?.dueDate || tenantRents[0].dueDate
  ...
  const handleOwnerRentAssign = (event) => {
    ...
    if (!ownerRentForm.dueDate) return showToast('Due date is required.', 'error')
  ...
  const columns = [
    ...
    { key: 'unit', label: 'Unit' },
    { key: 'joinedOn', label: 'Joined On', render: (row) => formatDate(row.joinedOn || joinedOn) },
    ...
    { key: 'amount', label: 'Amount', render: (row) => formatCurrency(row.amount) },
    {
      ...
      render: (row) => {
        const days = calculateRemainingDays(row.dueDate)
        return (
        ...
        isOwner ? (
          <>
            <Card>
              ...
              <p className="mt-1 text-sm text-soft">Select one of your tenant-occupied units and set due date and amount.</p>
              {!ownerTenantUnits.length ? (
                <p className="mt-4 text-sm text-rose-700">No tenant is assigned to your owned units yet.</p>
              ) : (
                <form className="mt-4 grid gap-3 md:grid-cols-4" onSubmit={handleOwnerRentAssign}>
                  <select
                  ...
                  <input
                    type="date"
                    className="input-base"
                    value={ownerRentForm.dueDate}
                    onChange={(event) => setOwnerRentForm((prev) => ({ ...prev, dueDate: event.target.value }))}
                  />
